...
def get_source_name(download_source:str)->str:
    ...
    result = subprocess.run(["yt-dlp", "--get-filename",
...
def get_best_mp4_single_file(download_source:str, description_file:str,
    ...
    subprocess.run(["yt-dlp", "--write-description", "-f",
...
def split_mp4(chapters:list, download_filename:str, download_name:str)->None:
    ...
    for current_index, current_chapter in enumerate(chapters):
        ...
        subprocess.run(["ffmpeg", "-ss", start_time, "-to", end_time,
                        "-i", download_filename, output_name])
        audio = EasyID3(output_name)
        audio["title"] = str(output_name.replace("_", " ").replace(".mp3", ""))
        audio["album"] = str(args.albumname)
        audio["artist"] = str(args.artistname)
        audio["tracknumber"] = str(current_index + 1)
        audio.save()


if __name__ == '__main__':

    parser = argparse.ArgumentParser(description='Download Albums from Youtube for use with Spotify and other music playing programs')
    ...
    for link_to_download in args.links:
        ...
        print("Feel free to edit the ID3 tags further in a program such as Mp3Tag (Unassociated)")